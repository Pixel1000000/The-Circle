...
Write-Step "cmake --preset $presetName -DTC_DEBUG_LOG=ON"
& cmake --preset $presetName -DTC_DEBUG_LOG=ON
...
Write-Host " Debug log build ready: $exePath" -ForegroundColor Green
Write-Host " Log will appear at:    build\debug_log.txt" -ForegroundColor Green
Write-Host "========================================================" -ForegroundColor Green

if ($Run -and (Test-Path $exePath)) {
    Write-Step "Starting TheCircle.exe"
    & $exePath
}
