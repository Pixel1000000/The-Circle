...
- [ ] **Волновая система с банком** — переработать `PlayState::spawnBiomeEnemies()`:
...
- [ ] **Настройки в PauseScreen** — добавить кнопку "Настройки" в `PauseScreen`. Вложенный экран:
...
- [ ] **Уникальные бонусы экипировки** — лайфстил, % урона по типу, бонусы к конкретным статам (упомянуто в GDD). Расширить `ArmorPieceStats`/`WeaponStats`.

### Мир
- [ ] **Препятствия и стены внутри биомов** — сейчас арена полностью открытая. Добавить статичные объекты с AABB-коллизией для тактического разнообразия.

### UI
- [ ] **Настройки из PauseScreen** — громкость звука и музыки, смена языка без выхода в главное меню.
