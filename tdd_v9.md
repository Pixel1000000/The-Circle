...
### Баланс
- [x] **`POINTS_PER_FRAGMENT`** — убрать совсем: фрагменты ключа не должны давать meta-очки, это мешает балансу.
  *(сделано: константа и её использование удалены из `MetaProgression.hpp/.cpp`, фрагменты не дают баллов)*
...
### Баланс
- [x] **`POINTS_PER_FRAGMENT`** — решено: фрагменты ключа не дают meta-очков, константа и логика удалены.
